...
from .models import Friendship, User, Balance
...
class FriendshipService:
    ...
    #     ```
    #     """
    #     balences = GroupBalance.objects.filter(Q(friend_owes=user) | Q(friend_owns=user))
    #     return balences
    ...
    #     balances = GroupBalance.objects.filter(
    ...
    def bulk_add_friends(user, friends):
        ...
        """
        friendships = [Friendship(friend_owes=user, friend_owns=friend) for friend in friends if friend != user]

        with transaction.atomic():
           return Friendship.objects.bulk_create(friendships, returning = True)
